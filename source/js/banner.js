(function() {
  ...
  var ManipularConfiguracoes = {
    ...
    pegarDadoNavegacaoAtual: function() {
      return ConfiguracoesBanner.navegacaoSelecionada;
    },

    selecionarEstadoAtualNavegacao: function(navegacao) {
      ConfiguracoesBanner.navegacaoSelecionada = navegacao;
    },

    mudarEstadoNavegacao: function(booleano) {
      ConfiguracoesBanner.navegacaoSelecionada.linkDesativado = booleano;
    },
  ...
  var BannerPrincipal = {
    ...
    resetaBanner: function(fechar) {
      ...
      var todosBoxes = document.getElementsByClassName('banner-secao'),
        ...
        dadosBoxes = ManipularConfiguracoes.dadosTodosBoxes(),
        btnFechar = fechar === undefined
          ? document.getElementById('botaoFecharBanner')
          : fechar,
        navegacao = this.containerNavegacaoBanner;
      ...
      for (j = 0; j < todosBoxesLength; j++) {
        ...
        //reseto a flag de tipo escolha
        dadosBoxes[j].flagTipoEscolha = "";

        //deixo todos os botões habilitados
        dadosBoxes[j].botaoHabilitado = true;
        todosBotoes.className = 'banner-btn';

        if (todosBoxes[j].contains(btnFechar)) {
          todosBoxes[j].removeChild(btnFechar);
        }

        if (todosBoxes[j].contains(navegacao)) {
          todosBoxes[j].removeChild(navegacao);
        }
      ...
      //zerando valor dos inputs que guardam a seleção do usuário
    ...
    mudaTextos: function(boxSelecionado) {
      ...
      var tipoEscolhaAtual = ManipularConfiguracoes.pegarDadoBoxAtual(),
        dados = tipoEscolhaAtual.flagTipoEscolha === "tipoEmpresa"
          ? ManipularConfiguracoes.dadosConfiguracoesTipoEmpresa()
          : ManipularConfiguracoes.dadosConfiguracoesCidade();
    ...
    adicionaOpcoes: function(boxSelecionado) {
      ...
      var o,
        ...
        opcoesHtml = tipoEscolhaAtual.flagTipoEscolha === "tipoEmpresa"
          ? this.todasOpcoesTipoEmpresa
          : this.todasOpcoesMunicipio,
    ...
    removeOpcoes: function(boxSelecionado, tipoEscolha) {
      var p,
        tipoEscolhaAtual = tipoEscolha === undefined
          ? ManipularConfiguracoes.pegarDadoBoxAtual()
          : tipoEscolha,
        opcoesHtml;

      if (tipoEscolhaAtual === tipoEscolha) {
        opcoesHtml = tipoEscolhaAtual === "tipoEmpresa"
          ? this.todasOpcoesTipoEmpresa
          : this.todasOpcoesMunicipio;
      } else {
        opcoesHtml = tipoEscolhaAtual.flagTipoEscolha === "tipoEmpresa"
    ...
    statusNavegacao: function() {
      ...
      for (m = 0; m < linksNavegacaoLength; m++) {
        var status = dadosDesativado[m].linkDesativado
          ? "banner-btn banner-lnk banner-lnk--desativado"
          : "banner-btn banner-lnk";
    ...
    verificaStatusBotao: function() {
      ...
      statusBtn = dadosDesativado.botaoHabilitado
        ? "banner-btn"
        : "banner-btn u-esconder";
